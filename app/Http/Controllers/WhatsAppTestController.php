...
class WhatsAppTestController extends Controller
{
     ...
     */
    public function testEmergencyAlert()
    {
        // Get first panic report or create dummy data
        $panicReport = PanicReport::with('user')->first();

        if (!$panicReport) {
            // Create dummy panic data for testing
            $dummyPanic = new PanicReport([
                'latitude' => -7.983908,
            ...
            $dummyPanic->user = $dummyUser;
            $panicReport = $dummyPanic;
        }
        ...
        if ($volunteers->isEmpty()) {
            return response()->json([
                'error' => 'No volunteers with phone numbers found for testing',
                'suggestion' => 'Add phone numbers to volunteers in database'
            ], 404);
        }

        $result = $this->whatsAppService->sendEmergencyAlert($panicReport, $volunteers);

        return response()->json([
            'test' => 'emergency_alert_simulation',
            'panic_report' => [
                'location' => [$panicReport->latitude, $panicReport->longitude],
                'description' => $panicReport->description,
                'reporter' => $panicReport->user->name ?? 'Unknown'
            ],
            'volunteers_notified' => $volunteers->count(),
     ...
     * Test bulk message
     */
    public function testBulkMessage(Request $request)
    {
        $volunteers = User::where('role', 'relawan')
            ->whereNotNull('no_telp')
            ->get();
        ...
        if ($volunteers->isEmpty()) {
            return response()->json([
                'error' => 'No volunteers with phone numbers found',
            ], 404);
        }

        $targets = $volunteers->map(function ($volunteer) {
            return [
